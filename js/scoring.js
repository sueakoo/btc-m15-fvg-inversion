...
function _scoreBlock9(mx, direction) {
  ...
  const othPos = isLong ? 'Короткие позиции' : 'Длинные позиции';
  const ownAdj = isLong ? 'длинных'          : 'коротких';
  const othAdj = isLong ? 'коротких'         : 'длинных';
  ...
  if (liq < 2) {
    ...
    if (doi >= 0.20) {
      if (!hasLimb) {
        ...
        score = 11; label = 'Встряска противоположной стороны + OI вошёл';
        comment = `${othPos} принудительно закрывались — их ликвидации стали топливом движения. OI вырос на этом давлении.`;
      }
    } else {
      score = 7; label = 'Ликвидации умеренные, подтверждения нет';
      if (!hasLimb) {
        comment = 'Ликвидации умеренные, нового OI нет — подтверждения нет.';
      } else if (against) {
        comment = `${ownPos} принудительно закрывались, нового OI нет — переоткрытия позиции не последовало.`;
      } else {
    ...
    if (doi >= 0.20) {
      if (!hasLimb) {
        ...
        score = 11; label = 'Ликвидации противоположной стороны + OI вошёл';
        comment = `${othPos} принудительно закрывались — закрытие создало ${action}. OI подтвердил вход.`;
      }
    } else if (doi >= 0) {
      score = 7; label = 'Ликвидации без роста OI';
      if (!hasLimb) {
        comment = 'Значимые ликвидации без нового OI — подтверждения нет.';
      } else if (against) {
        comment = `Значимые ликвидации ${ownAdj} позиций без переоткрытия — новый OI не появился.`;
      } else {
        comment = `Значимые ликвидации ${othAdj} позиций без нового OI — движение на закрытиях не подкреплено.`;
      }
    } else {
      score = 4; label = 'Ликвидации + OI снижается';
      if (!hasLimb) {
        comment = 'Ликвидации при снижении OI — ресурс покидает уровень.';
      } else if (against) {
        comment = `${ownPos} закрывались принудительно при снижении OI — ресурс покидает уровень.`;
      } else {
        comment = `${othPos} закрывались принудительно при снижении OI — движение без подтверждения.`;
      }
    }
  } else {
    if (doi >= 0.20) {
      ...
      if (!hasLimb) {
        ...
        comment = `Массовые принудительные закрытия ${othAdj} позиций усилили движение — OI вырос, направление подтверждено.`;
      }
    } else if (doi >= 0) {
      score = 2; label = 'Высокая доля стопов, OI не растёт';
      if (!hasLimb) {
        comment = 'Массовые ликвидации без нового OI — позиция не набрана.';
      } else if (against) {
        comment = `Массовые принудительные закрытия ${ownAdj} позиций — нового OI нет, позиция не набрана.`;
      } else {
...
function _redFlags(b, mx, det) {
  ...
  if (b.block4.score <= 6 && b.total >= 70) {
    flags.push('Слабая механика захвата — инверсионная прошла без набора OI при высоком балле. Риск пробоя зоны при тесте повышен');
  }
  ...
  if (b.block4.score <= 6 && b.block9.score >= 16) {
    const actor = det.direction === 'long' ? 'покупателя' : 'продавца';
    flags.push(`Позиция набрана до инверсии — в момент пересечения FVG нового ${actor} не было. При тесте зоны риск пробоя повышен — защитника здесь нет`);
  }
...
const _MECH_INTRO = {
  ...
  resistance: {
    ...
    working:  'CVD сильный в сторону инверсии, тело слабое — встречная сторона на М15 не сдавалась. Зона пробита с сопротивлением.',
